 ...
 * 更新描述   ${TODO}
 */
public class ActivityMyFragment extends BaseFragment implements OnRefreshListener, OnLoadmoreListener, ActListAdapter.OnRecyclerViewItemClickListener {
    private SmartRefreshLayout mSrlActMy;
    private RecyclerView mRvActMyRecyView;
    private ArrayList<ActListBean.RowsBean> mActRows;

    private static final String TAG = "ActivityMyFragment";
    ...
    public View initView() {
        View view = View.inflate(mActivity, R.layout.fragment_activity_my, null);
        mSrlActMy = view.findViewById(R.id.srl_act_my);
        mRvActMyRecyView = view.findViewById(R.id.rv_act_my);

        LinearLayoutManager layoutManager = new LinearLayoutManager(mActivity);
        layoutManager.setOrientation(LinearLayoutManager.VERTICAL);//设置为垂直布局，这也是默认的
        mRvActMyRecyView.setLayoutManager(layoutManager);// 设置布局管理器
        mRvActMyRecyView.addItemDecoration(new DividerItemDecoration(mActivity, DividerItemDecoration.VERTICAL));
        //下拉刷新 上拉加载
        mSrlActMy.setOnRefreshListener(this);
        mSrlActMy.setOnLoadmoreListener(this);
...
//                refreshlayout.setLoadmoreFinished(false);//可以出发加载更多事件
    ...
    private void _processJson(String result) {
        ...
        mRvActMyRecyView.setAdapter(actListAdapter);
        actListAdapter.setOnItemClickListener(this);
    }

    private void _queryActList() {
        RequestParams params = new RequestParams(Url.QUERY_ACTIVITY_MY_URL);
    ...
    public void onItemClick(View view, ArrayList<ActListBean.RowsBean> data) {
        int position = mRvActMyRecyView.getLayoutManager().getPosition(view);
