 ...
 * 更新描述   ${TODO}
 */
public class ServiceListFragment extends BaseFragment implements OnRefreshListener, OnLoadmoreListener, RequestMyAdapter.OnRecyclerViewItemClickListener {
    private SmartRefreshLayout mSrlSerList;
    private RecyclerView mRvSerListRecyView;
    ...
    public View initView() {
        View view = View.inflate(mActivity, R.layout.fragment_service_list, null);
        mSrlSerList = view.findViewById(R.id.srl_ser_list);
        mRvSerListRecyView = view.findViewById(R.id.rv_ser_list);
        LinearLayoutManager layoutManager = new LinearLayoutManager(mActivity);
        layoutManager.setOrientation(LinearLayoutManager.VERTICAL);//设置为垂直布局，这也是默认的
        mRvSerListRecyView.setLayoutManager(layoutManager);// 设置布局管理器
        mRvSerListRecyView.addItemDecoration(new DividerItemDecoration(mActivity, DividerItemDecoration.VERTICAL));
        //下拉刷新 上拉加载
        mSrlSerList.setOnRefreshListener(this);
        mSrlSerList.setOnLoadmoreListener(this);
...
//                refreshlayout.setLoadmoreFinished(false);//可以出发加载更多事件
    ...
    private void _processJson(String result) {
        ...
        mRvSerListRecyView.setAdapter(reqMyAdapter);
        reqMyAdapter.setOnItemClickListener(this);
    }

    private void _queryMyReq() {
        RequestParams params = new RequestParams(Url.QUERY_SERVICE_LIST_URL);
    ...
    public void onItemClick(View view, ArrayList<ReqMyBean.RowsBean> data) {
        int position = mRvSerListRecyView.getLayoutManager().getPosition(view);
